...
export function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-4 rounded-4xl bg-surface p-4 ring-1 ring-line-strong transition-all hover:-translate-y-0.5 hover:ring-lavender-300 hover:shadow-soft sm:gap-5 sm:p-5"
    >
      <OrderThumbs order={order} muted={closed} />

      <span className="min-w-0 flex-1">
        <span className={cn("block truncate text-sm font-semibold", closed ? "text-muted" : "text-ink")}>
